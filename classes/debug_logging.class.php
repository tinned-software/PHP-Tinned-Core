    ...
    **/
    public function set_catch_php_errors($enable)
    {
        if($enable == true)
        {
            ...
            // Set this class' "catch_php_error" method as PHP's error handling function
            $old_handler = $this->old_error_handle;
            $this->old_error_handle = set_error_handler(array (&$this, 'catch_php_error'));
            
            // Ignore old error handlers that were created by this method
            if(
                (is_object($this->old_error_handle) === TRUE && get_class($this->old_error_handle) === 'Closure')
                ||
                (is_array($this->old_error_handle) === TRUE && $this->old_error_handle === array(&$this, 'catch_php_error'))
              )
            {
                $this->old_error_handle = $old_handler;
            }
        ...
        // $enable is FALSE and the old error handler was NULL (need to switch back to the built in error handler)
        else
        {
            // We can't use restore_error_handler() because we dont know how many 
            // times the error handler has been changed, so set an error handler
            // which returns FALSE, causing the PHP error handler to be used
